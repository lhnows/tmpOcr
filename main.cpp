...
void *wii_loop(void *arg)
{
	while(1)
	{
		char key = Cwiiuse::getInstance()->getAction(10000);
		if ('Z'==key) key =' ';
		printf("The Action Key is %c \n",key);
		switch(key)
		{
		case '1':
			OcrUI::getInstance()->setRumbleOrSound(1);
			break;
		case '2':
			OcrUI::getInstance()->setRumbleOrSound(0);
			break;
		default:
			break;
		}
...
*/
	...
	try
	{
		...
		while (_loop)
		{
			...
			ldt_arr.count = textDetected;
			for (int i = 0; i < textDetected; i++)
			{
				ldt_arr.dtdata[i].text = textStrSrc[i];
				ldt_arr.dtdata[i].dist = (float)depthArr[i] / 1000;
			}
			OcrUI::getInstance()->process(ldt_arr);
#if 0
			string strSpe[50];
			string strDist[50];
			if (0 == conuse)
